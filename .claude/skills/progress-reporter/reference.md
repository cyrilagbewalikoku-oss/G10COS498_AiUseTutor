...
2. **Highlight achievements and milestones.** Scan the interaction and assessment history for noteworthy accomplishments.
...
3. **Identify areas needing attention.** Flag dimensions that meet any of the following criteria:
...
4. **Show trajectory per dimension.** Provide a brief narrative interpretation of each dimension's trend.
...
5. **Suggest next steps.** Recommend 2-3 specific activities that are aligned with the learner's gaps and stated goals.
...
6. **If near level-up threshold, highlight proximity.** When the learner's lowest dimension score is within 0.5 points of the next level's requirement, explicitly call this out to motivate continued effort.
...
|---|---|---|
| `progressReport` | `string` | The full formatted progress report as shown to the user, including progress bars, achievements, trajectory, and suggestions |
...
| `suggestedNextSteps` | `string[]` | 2-3 specific activity recommendations with rationale |
...
- **Avoid information overload.** Cap the report at the essentials: progress bars, 2-3 highlights, 1-2 areas to focus on, and 2-3 suggestions. If the user wants more detail, they can ask.
